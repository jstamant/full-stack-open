const { test, describe } = require('node:test')
const assert = require('node:assert')
const listHelper = require('../utils/list_helper')

const sampleList = [
  {
    _id: "5a422a851b54a676234d17f7",
    title: "React patterns",
    author: "Michael Chan",
    url: "https://reactpatterns.com/",
    likes: 7,
    __v: 0
  },
  {
    _id: "5a422aa71b54a676234d17f8",
    title: "Go To Statement Considered Harmful",
    author: "Edsger W. Dijkstra",
    url: "http://www.u.arizona.edu/~rubinson/copyright_violations/Go_To_Considered_Harmful.html",
    likes: 5,
    __v: 0
  },
  {
    _id: "5a422b3a1b54a676234d17f9",
    title: "Canonical string reduction",
    author: "Edsger W. Dijkstra",
    url: "http://www.cs.utexas.edu/~EWD/transcriptions/EWD08xx/EWD808.html",
    likes: 12,
    __v: 0
  },
  {
    _id: "5a422b891b54a676234d17fa",
    title: "First class tests",
    author: "Robert C. Martin",
    url: "http://blog.cleancoder.com/uncle-bob/2017/05/05/TestDefinitions.htmll",
    likes: 10,
    __v: 0
  },
  {
    _id: "5a422ba71b54a676234d17fb",
    title: "TDD harms architecture",
    author: "Robert C. Martin",
    url: "http://blog.cleancoder.com/uncle-bob/2017/03/03/TDD-Harms-Architecture.html",
    likes: 0,
    __v: 0
  },
  {
    _id: "5a422bc61b54a676234d17fc",
    title: "Type wars",
    author: "Robert C. Martin",
    url: "http://blog.cleancoder.com/uncle-bob/2016/05/01/TypeWars.html",
    likes: 2,
    __v: 0
  }
]

describe('total likes', () => {
  test('of an empty list', () => {
    assert.strictEqual(listHelper.totalLikes([]), 0)
  })
  test('of one single blog post in a list', () => {
    assert.strictEqual(listHelper.totalLikes([sampleList[0]]), 7)
  })
  test('of a list of blog posts', () => {
    assert.strictEqual(listHelper.totalLikes(sampleList), 36)
  })
})

describe('favorite blog', () => {
  test('of an empty list', () => {
    assert.strictEqual(listHelper.favoriteBlog([]), undefined)
  })
  test('of one single blog post in a list', () => {
    const value = {
      _id: "5a422a851b54a676234d17f7",
      title: "React patterns",
      author: "Michael Chan",
      url: "https://reactpatterns.com/",
      likes: 7,
      __v: 0
    }
    assert.deepStrictEqual(listHelper.favoriteBlog([sampleList[0]]), value)
  })
  test('of a list of blog posts', () => {
    const value = {
      _id: "5a422b3a1b54a676234d17f9",
      title: "Canonical string reduction",
      author: "Edsger W. Dijkstra",
      url: "http://www.cs.utexas.edu/~EWD/transcriptions/EWD08xx/EWD808.html",
      likes: 12,
      __v: 0
    }
    assert.deepStrictEqual(listHelper.favoriteBlog(sampleList), value)
  })
})

describe('most blog posts', () => {
  test('of an empty list', () => {
    assert.strictEqual(listHelper.mostBlogs([]), undefined)
  })
  test('of one single blog post in a list', () => {
    const value = { author: 'Edsger W. Dijkstra', blogs: 1 }
    assert.deepStrictEqual(listHelper.mostBlogs([sampleList[1]]), value)
  })
  test('of a list of blog posts', () => {
    const value = { author: 'Robert C. Martin', blogs: 3 }
    assert.deepStrictEqual(listHelper.mostBlogs(sampleList), value)
  })
})

describe('most likes', () => {
  test('of an empty list', () => {
    assert.strictEqual(listHelper.mostLikes([]), undefined)
  })
  test('of one single blog post in a list', () => {
    const value = { author: 'Michael Chan', likes: 7 }
    assert.deepStrictEqual(listHelper.mostLikes([sampleList[0]]), value)
  })
  test('of a list of blog posts', () => {
    const value = { author: 'Edsger W. Dijkstra', likes: 17 }
    assert.deepStrictEqual(listHelper.mostLikes(sampleList), value)
  })
})
